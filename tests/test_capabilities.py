...
class CapabilitiesTests(unittest.TestCase):
    def test_known_families(self) -> None:
        ...
        self.assertTrue(caps_for(v3).has_battery_percentage)
        self.assertTrue(caps_for(mini).has_battery_percentage)

    def test_unknown_falls_back_to_v2(self) -> None:
        future = Device(device_id="z", name="?", device_family="v4")
        caps = caps_for(future)
        self.assertTrue(caps.has_display)
        self.assertFalse(caps.has_battery_percentage)
